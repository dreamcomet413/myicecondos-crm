class Activity
  ...
  def self.most_active_leads
    users = ActiveRecord::Base.connection.execute("select u.email, v.user_id, count(v.id) as num from #{APP_CONFIG[:alli_db]}.users u left join #{APP_CONFIG[:alli_db]}.visits v on u.id = v.user_id where v.user_id IS NOT NULL group by v.user_id order by num DESC limit 20;")
