...
class Campaign < ActiveRecord::Base
  ...
  validates_uniqueness_of :source, allow_nil: true
  ...
  def schedule_lead_emails lead
    return unless status == "started"
    start_time = DateTime.now if starts_on.nil? || (starts_on.beginning_of_day < DateTime.now)
    start_time ||= starts_on.beginning_of_day
    campaign_emails.each do |e|
      e.send_and_record(lead, (start_time + e.send_in_minutes.try(:minutes)))
    end
  end
