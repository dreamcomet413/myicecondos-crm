...
class CampaignsController < EntitiesController
  ...
  #----------------------------------------------------------------------------
  def index
    @campaigns = get_campaigns(:page => params[:page], :per_page => params[:per_page], user_id: current_user.id)
  ...
  def create
    ...
    respond_with(@campaign) do |format|
      if @campaign.save
        ...
        @campaigns = get_campaigns(user_id: current_user.id)
  ...
  def redraw
    ...
    @campaigns = get_campaigns(:page => 1, :per_page => params[:per_page], user_id: current_user.id)
...
private
  ...
  def respond_to_destroy(method)
    if method == :ajax
      ...
      @campaigns = get_campaigns(user_id: current_user.id)
      if @campaigns.blank?
        @campaigns = get_campaigns(:page => current_page - 1, user_id: current_user.id) if current_page > 1
